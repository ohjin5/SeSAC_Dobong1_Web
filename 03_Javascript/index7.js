...
// 실습3 
let fruit1 = ["사과", "딸기", "파인애플", "수박", "참외", "오렌지", "자주", "망고"];
let fruit2 = ["수박", "사과", "참외", "오렌지", "파인애플", "망고"];

// 동일한 요소만을 가지는 배열 생성
let same = fruit1.filter(fruit => fruit2.includes(fruit));

console.log("동일한 요소를 가지는 배열: " + same);

// 서로 다른 요소만을 가지는 배열 생성
let diff = fruit1.filter(fruit => !fruit2.includes(fruit))
    .concat(fruit2.filter(fruit => !fruit1.includes(fruit)));

console.log("서로 다른 요소를 가지는 배열: " + diff);
